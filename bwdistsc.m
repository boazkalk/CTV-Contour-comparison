function [D,Index]=bwdistsc(bw,aspect)
...
D=cell(1,shape(3)); 
for k=1:shape(3) D{k}=zeros(shape(1:2)); end
Indexes=cell(1,shape(3)); 
for k=1:shape(3) Indexes{k}=zeros(shape(1:2)); end

%%%%%%%%%%%%% scan along XY %%%%%%%%%%%%%%%%
for k=1:shape(3)    
    if(iscell(bw)) bwXY=bw{k}; else bwXY=bw(:,:,k); end %bwXY is kth XY matrix in z direction
    ...
    if isempty(find(ind))==1
        ...
        Indexes{k} = ind+((k-1)*shape(1)*shape(2));
    end
end
%%%%%%%%%%%%% scan along Z %%%%%%%%%%%%%%%%
D1=cell(size(D));
...
Indexes_2 = cell(size(Indexes));
for k=1:shape(3) 
  Indexes_2{k}=zeros(shape(1:2)); 
end
...
for k=1:shape(3)
    ...
    for kk=p-1:-1:1
        ...
        indtmp=Indexes{k}(idx_lower);
       
        % these pixels are to be updated
        L=D1{kk}(idx_lower)>dtmp;
        map_lower(idx_lower)=L;
        D1{kk}(idx_lower(L))=dtmp(L);
        Indexes_2{kk}(idx_lower(L)) = indtmp(L);
...
if(iscell(bw))
    ...
    Index=zeros(shape);
    for k=1:shape(3) D(:,:,k)=sqrt(D1{k}); end
    for k=1:shape(3) Index(:,:,k)=Indexes_2{k}; end
end
end